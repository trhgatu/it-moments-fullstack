...
function Sidenav({ color }) {
  ...
  const { role: userRole } = useUser(); // Đổi tên biến từ `role` thành `userRole`
    ...
    <div className="sidenav">
      ...
      <Menu theme="light" mode="inline">
        {menuItems.map(({ key, label, path, permission }) => {
          const hasPermission = permission ? userRole?.permissions.includes(permission) : true; // Kiểm tra quyền

          return hasPermission ? (
            <Menu.Item key={key}>
              <NavLink to={path}>
                <span className="icon" style={{ background: page === path ? color : "" }}>
                  {/* SVG Icon */}
